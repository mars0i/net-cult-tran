...
to hear-rumor
  ...
  ifelse eight-mode?
    [set homeys neighbors
     set num-homeys 8]
    [set homeys neighbors4
     set num-homeys 4]
    
  if ((count homeys with [times-heard > 0]) / num-homeys) >= q
    [set just-heard? true]
end

to update-patch-stats [when]  ;; patch procedure
  if first-heard = -1
...
[This is a slightly hacked version of the NetLogo models lib Rumor Mill.  This model implements the patch update procedure from Stephen Morris's "Contagion", The Review of Economic Studies 2000 67(1) 57-78, as presented in Vega-Redondo 2007.  This means that a patch acquires the "rumor" if and only if the proportion of its neighbors (Moore 8 or von Neuman 4) who have the rumor already is greater than a parameter q.  All of the documentation not in square brackets is from the original Rumor Mill model. -Marshall Abrams]
...
[Vega-Redondo gives an example that shows that you can get contagion on a lattice with a q greater than 1/number-of-neighbors.  To implement this example, use the Moore 8-neighborhood, and initialize the lattice with a square of 9.  Then q can be as high as 3/8, and contagion will occur.  To make the 9-square, set q to .125 and step once.  Then set q to .375 to see the effect.]
